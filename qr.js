...
function _createOrUpdateQRCode(qrText, qrcodeDiv, downloadButtonsDiv) {
    ...
    if (!qrCode) {
        qrCode = new QRCodeStyling({
            width: 350, // Increased to accommodate border
            height: 350, // Increased to accommodate border
            type: "svg",
            data: qrText,
            margin: 50, // Add margin for the border plugin
        ...
        // Configure border plugin options
        const borderOptions = {
            round: 0.77, // Roundness (0-1)
            thickness: 30, // Border thickness
            color: "#55298a", // Border color matching your theme
            decorations: {
                bottom: {
                    type: "text",
                    value: "SCAN ME",
                    style: "font: bold 24px Arial; fill: #55298a; text-anchor: middle;"
                }
            },
            borderInner: {
                color: "#FFFFFF",
        ...
        // Set license key (replace with your actual key for production)
        // QRBorderPlugin.setKey("your_license_key");
        
        qrCode.append(qrcodeDiv);
        
        // Remove the setTimeout for border radius since the plugin handles styling
